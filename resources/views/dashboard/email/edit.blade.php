...
@section('content')
    ...
    <div class="main-content">
        <div class="page-header">
            <h2 class="header-title">Edit Email</h2>
            <div class="header-sub-title">
                <nav class="breadcrumb breadcrumb-dash">
                    ...
                    <span class="breadcrumb-item active">Edit - {{ $email->subject }}</span>
                </nav>
            </div>
        </div>
        <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="m-t-25">
                                    <form method="post" action="{{ route('email.update', $email->uuid) }}" enctype="multipart/form-data">
                                        @csrf
                                        {{ method_field('put') }}
                                        <div class="form-row">
                                            <div class="form-group col-md-6">
                                                <label for="inputEmail4">Email Subject</label>
                                                <input type="text" name="subject" class="form-control" id="subject" placeholder="Subject" autocomplete="off" required value="{{ $email->subject }}">
                                            </div>
                                            <div class="form-group col-md-6">
                                                <label for="sender">Email Sender ('NYCC Membership' if left blank)</label>
                                                <input type="text" name="sender" class="form-control" id="sender" placeholder="NYCC Membership" autocomplete="off" required value="{{ $email->sender }}">
                                            </div>
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-4">
                                                ...
                                                <select id="inputState" class="form-control email_type" name="type" required>
                                                    ...
                                                    <option value="private" {{ $email->private?'selected':''  }}>Private (Single Recipient)</option>
                                                    <option value="public" {{ $email->private?'':'selected' }}>Bulk (Mail List Recipient)</option>
                                                </select>
                                            </div>
                                            <div class="form-group col-md-4 email_field" style="display: {{ $email->private?'block':'none' }}">
                                                <label for="inputPassword4">Recipient Email</label>
                                                <input type="email" name="email" class="form-control" placeholder="Recipient Email" autocomplete="off" value="{{ $email->recipient }}">
                                            </div>
                                            <div class="form-group col-md-4">
                                                <label for="inputPassword4">Email Title</label>
                                                <input type="text" name="title" class="form-control" id="title" placeholder="Title" autocomplete="off" required value="{{ $email->title }}">
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea cols="" rows="24" type="text" name="body" class="form-control myfield">{!! $email->body !!}</textarea>
                                        </div>
